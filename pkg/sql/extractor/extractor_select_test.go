...
func TestExtractColumns(t *testing.T) {
	...
	tests := []struct {
		...
		{"select key, value from example where id = 20 AND key IN ( 'a', 'b', 'c' );",
			[]string{"SELECT|public.example.key", "SELECT|public.example.value", "WHERE|public.example.id", "WHERE|public.example.key"}},
		// escaped apostrophes
		{"SELECT translate(name, '''', '' ) as name FROM people WHERE id = 0;",
			[]string{"SELECT|public.people.name", "WHERE|public.people.id"}},
		// coalesce
		...
		// Select: where clause
		{"select id from users where id = 96;",
			[]string{"SELECT|public.users.id", "WHERE|public.users.id"}},
		// {"select id from users where id = 852 and customer_id = 74", []string{"id", "name"}},
		// {"select id from users where id = 963 and customer_id > 74;", []string{"id", "name"}},
		...
		{"select id from users where id = 19 group by id, name",
			[]string{"SELECT|public.users.id", "WHERE|public.users.id", "GROUP_BY|public.users.id", "GROUP_BY|public.users.name"}},
		...
		{"select id from users order by id;",
			[]string{"SELECT|public.users.id", "ORDER|public.users.id"}},
		...
		// {"SELECT u.* from users u where u.id IN (517);", []string{"id", "name"}},
		...
		// TODO: THIS IS A BUG: we need to look at all of this again.
		// Select: EXISTS operator. In this case, NOT is a prefix operator
		{"select id from cars where exists (select id from colors where car_id = cars.id);",
			[]string{"SELECT|public.UNKNOWN.id", "WHERE|public.UNKNOWN.car_id", "WHERE|public.cars.id"}},
		...
		{"select id from users where data->'name' = 'brian';",
			[]string{"SELECT|public.users.id", "WHERE|public.users.data"}},
		...
		// {"select * from users where metadata @> '{\"age\": 17}';", []string{"id", "name"}},
		// {"select * from users where metadata <@ '{\"age\": 18}';", []string{"id", "name"}},
		// {"select * from users where metadata ? '{\"age\": 19}';", []string{"id", "name"}},
		// {"select * from users where metadata ?| '{\"age\": 20}';", []string{"id", "name"}},
		// {"select * from users where metadata ?& '{\"age\": 21}';", []string{"id", "name"}},
		// {"select * from users where metadata || '{\"age\": 22}';", []string{"id", "name"}},
		...
		{"select * from (select id from a) b order by id",
			[]string{"SELECT|public.a.id", "ORDER|public.a.id"}},
		...
		// TODO: Need to extract wildcards
		...
		// {"SELECT * FROM ( VALUES (41, 1), (85, 2), (43, 3), (44, 4), (45, 5), (46, 6) ) AS t ( id, type_id )", []string{"id", "name"}},
	...
	for _, tt := range tests {
		...
		for i, s := range program.Statements {
			...
			assert.Equal(t, len(tt.columns), len(r.ColumnsInQueries), "input: %s\nExpected %d columns, got %d", tt.input, len(tt.columns), len(r.ColumnsInQueries))

			for fqcn := range r.ColumnsInQueries {
				// if column.Clause == token.SELECT {
				assert.Contains(t, tt.columns, fqcn, "input: %s\nColumn %s not found in %v", tt.input, fqcn, tt.columns[i])
				// }
			}
		}
	}
	t2 := time.Now()
	timeDiff := t2.Sub(t1)
	fmt.Printf("TestExtractColumns, Elapsed Time: %s\n", timeDiff)
}
...
func TestExtractSelectedTables(t *testing.T) {
	...
	tests := []struct {
		...
		// Select: where clause
		{"select id from users where id = 419;",
			[][]string{{"public.users"}}},
		// {"select id from users where id = 418 and customer_id = 74", [][]string{{"id", "name"}}},
		// {"select id from users where id = 417 and customer_id > 74;", [][]string{{"id", "name"}}},
		// {"select id from users where name = 'brian';", [][]string{{"id", "name"}}},
		// {"select id from users where name = 'brian'", [][]string{{"id", "name"}}},
		// {"select id from users where name is null", [][]string{{"id", "name"}}},
		// {"select id from users where name is not null", [][]string{{"id", "name"}}},
		...
		// Select: combined clauses
		{"select id from users where id = 55 group by id, name",
			[][]string{{"public.users"}}},
		// {"select id from customers join addresses on id = customer_id where id = 46 group by id;", [][]string{{"id", "name"}}},
		...
		// {"select position('b' IN 'brian') as foo from cars", [][]string{{"id", "name"}}}, // in inside a function call
		// {"select count(1) from cars c left join models m ON ( position((c.key) in m.formula)<>0 )", [][]string{{"id", "name"}}},
		// {"SELECT u.* from users u where u.id IN (482);", [][]string{{"id", "name"}}},
		...
		// {"select id from users where data#>>'{name,first}' = 'brian';", [][]string{{"id", "name"}}},
		// {"select id from users where data#>>'{name,first}' = 'brian' and data#>>'{name,last}' = 'broderick';", [][]string{{"id", "name"}}},
		// {"select * from users where metadata @> '{\"age\": 31}';", [][]string{{"id", "name"}}},
		// {"select * from users where metadata <@ '{\"age\": 32}';", [][]string{{"id", "name"}}},
		// {"select * from users where metadata ? '{\"age\": 33}';", [][]string{{"id", "name"}}},
		// {"select * from users where metadata ?| '{\"age\": 34}';", [][]string{{"id", "name"}}},
		// {"select * from users where metadata ?& '{\"age\": 35}';", [][]string{{"id", "name"}}},
		// {"select * from users where metadata || '{\"age\": 36}';", [][]string{{"id", "name"}}},
		...
		// {"select id from extra where number ilike e'001';", [][]string{{"id", "name"}}}, // escapestring of the form e'...'
		// {"select id from extra where number ilike E'%6%';", [][]string{{"id", "name"}}}, // escapestring of the form E'...'
		// {"SELECT * FROM ( VALUES (41, 1), (236, 2), (43, 3), (44, 4), (45, 5), (46, 6) ) AS t ( id, type_id )", [][]string{{"id", "name"}}},
